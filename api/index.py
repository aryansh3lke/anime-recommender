...
# Load models
knn_model = pickle.load(open('./api/models/knn_model.pkl','rb'))
svd_model = pickle.load(open('./api/models/svd_model.pkl','rb'))

rid_to_name, name_to_rid = projutils.read_item_names()
RECOMMENDATION_COUNT = 10

# Load anime scores:
anime_scores_df = pd.read_csv('./api/data/anime.csv', index_col='MAL_ID', usecols=['MAL_ID', 'Name', 'Score'])

# Load anime images:
anime_images = projutils.read_anime_images()

# Load SVD sample data
svd_samples_df = pd.read_csv('./api/data/frontend_svd_sample.csv')
...
#region KNN route
@app.route('/api/python/predict_knn', methods=['GET'])
def predict():
...
#region SVD Route
@app.route('/api/python/predict_svd', methods=['POST'])
def predict2():
