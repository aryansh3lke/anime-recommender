...
# Load anime scores:
anime_scores_df = pd.read_csv(
    "./data/anime.csv", index_col="MAL_ID", usecols=["MAL_ID", "Name", "Score"]
)
...
# region KNN route
@app.route("/api/get_knn_recommendations", methods=["GET"])
def get_knn_recommendations():
    ...
    try:
        prediction_ids = knn_model.get_neighbors(
            knn_model.trainset.to_inner_iid(int(raw_id)), k=3 * RECOMMENDATION_COUNT
        )  # this returns a list e.g. [127.20488798], so pick first element [0]
        prediction_ids = list(
            (knn_model.trainset.to_raw_iid(inner_id) for inner_id in prediction_ids)
        )
    ...
    for pred in predictions:
        recommendation_list.append(
            {"name": pred[0], "score": pred[1], "image": pred[2]}
        )
...
def predict_svd():
        ...
        try:
            ...
            if name not in name_to_rid:
                return render_template(
                    "index.html",
                    svd_error=f"Input Error: Invalid Anime Name! ({components[0].strip()})",
                    svd_input=user_profile_string,
                )
            ...
            return render_template(
                "index.html",
                svd_error=f'Input Error: Please enter ratings one per line with the format "anime name \\ rating"',
                svd_input=user_profile_string,
            )
    ...
    try:
        ...
        predict_anime_ids = [
            int(id) for id in predict_anime_ids if int(id) not in anime_ids
        ]
        ...
        # Get top N
        result = projutils.get_top_n_string(
            predictions, RECOMMENDATION_COUNT, rid_to_name
        )
        return render_template(
            "index.html", prediction_svd=f"{result}", svd_input=user_profile_string
        )
    except Exception as exc:
        print(exc)
        return render_template(
            "index.html",
            svd_error=f"Model Error: Something went wrong with model fitting",
            svd_input=user_profile_string,
        )
...
def get_anime_suggestions():
    ...
    return (
        jsonify({"suggestions": projutils.get_myanimelist_suggestions(input_anime)}),
        200,
    )
...
def get_anime_recommendations():
    ...
    return (
        jsonify(
            {"recommendations": projutils.get_myanimelist_recommendations(anime_name)}
        ),
        200,
    )
